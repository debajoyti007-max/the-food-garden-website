...
export default function Layout() {
  ...
  return (
    <div className="app-shell" style={{ minHeight: '100vh', background: '#121214', color: '#fafaf9', position: 'relative' }}>
      ...
      {/* 🏝️ Dynamic Island Pill (Only for customers with active cart/order) */}
      {!isRider && <DynamicIsland />}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span>
            {isRider
              ? '🛵 Delivery Partner Terminal — Drive Safe'
              : 'Bhabanipur, Nandakumar–Digha Highway'}
          </span>
        </div>
      ...
      >
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link to={isRider ? '/rider' : '/'} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <div
              style={{
                width: '44px',
                height: '44px',
                ...
                placeItems: 'center',
                fontSize: '1.4rem',
                boxShadow: isRider ? '0 4px 16px rgba(16, 185, 129, 0.35)' : '0 4px 16px rgba(245, 158, 11, 0.35)',
              }}
            >
              {isRider ? '🛵' : '🍽️'}
            </div>
            <div>
              ...
              <span style={{ fontSize: '0.75rem', color: isRider ? '#34d399' : '#f59e0b', fontWeight: 700, letterSpacing: '0.02em' }}>
                {isRider ? 'Rider Delivery Terminal' : STORE_TAGLINE}
              </span>
            </div>
          </Link>
          ...
          <nav className="desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            {isRider ? (
              // 🛵 RIDER EXCLUSIVE NAV: Focus on delivery work
              <>
                <NavLink
                  to="/rider"
                  style={({ isActive }) => ({
                    color: isActive ? '#34d399' : '#d6d3d1',
                    textDecoration: 'none',
                    fontWeight: 700,
                    fontSize: '0.9rem',
                  })}
      ...
      <a
        href={`https://wa.me/${SUPPORT_WHATSAPP}?text=${encodeURIComponent(
          isRider
            ? `নমস্কার The Food Garden, আমি রাইডার ${user?.name || ''} ডেলিভারি সংক্রান্ত বিষয়ে যোগাযোগ করছি।`
            : 'নমস্কার The Food Garden (TFG), আমি অর্ডার / টেবিল সম্পর্কে জানতে চাই!'
        )}`}
        target="_blank"
        rel="noreferrer"
      ...
      >
        {isRider ? (
          // 🛵 RIDER BOTTOM BAR
          <>
            <NavLink to="/rider" style={({ isActive }) => ({ display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none', color: isActive ? '#34d399' : '#a1a1aa', fontSize: '0.72rem', fontWeight: 700, gap: '2px' })}>
              <span style={{ fontSize: '1.25rem' }}>🛵</span>
              <span>Deliveries</span>
            </NavLink>
            <NavLink to="/track" style={({ isActive }) => ({ display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none', color: isActive ? '#34d399' : '#a1a1aa', fontSize: '0.72rem', fontWeight: 700, gap: '2px' })}>
              <span style={{ fontSize: '1.25rem' }}>📍</span>
              <span>Track</span>
            </NavLink>
            <NavLink to="/orders" style={({ isActive }) => ({ display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none', color: isActive ? '#34d399' : '#a1a1aa', fontSize: '0.72rem', fontWeight: 700, gap: '2px' })}>
              <span style={{ fontSize: '1.25rem' }}>📦</span>
              <span>History</span>
            </NavLink>
            <NavLink to="/profile" style={({ isActive }) => ({ display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none', color: isActive ? '#34d399' : '#a1a1aa', fontSize: '0.72rem', fontWeight: 700, gap: '2px' })}>
              <span style={{ fontSize: '1.25rem' }}>👤</span>
              <span>Profile</span>
            </NavLink>
          </>
